...
public class ConnectionImpl extends AbstractConnection {
	...
	public String receive() throws ConnectException, IOException {
        ...
        if (isValid(received) && received.getSeq_nr() > lastValidPacketReceived.getSeq_nr() && received.getSeq_nr() < (lastValidPacketReceived.getSeq_nr() + 30)) { //slik at samme pakke ikke oppfattes som mottatt to ganger. Bruker kun > (og ikke == +1) siden acker som ikke mottas kan lage uregelmessige hopp i sekvensnr. Satt en liten begrensning på hvor store hopp som godtas, siden headerfeil kan også lage hopp i sekvensnr OG DET GJENKJENNES IKKE I CHECKSUM
        	...
        	sendAck(lastValidPacketReceived, false); // Ack pŒ forrige gyldige, om ikke det er siste sendt mŒ ny bli sendt.
        	return null; 
        }
	}
	...
	public void close() throws IOException {
		try {
			if(state != State.ESTABLISHED) //mŒ ha conecction Œ lukke
			...
			int timeUsed = 0;
			while(answer == null || answer.getFlag() == Flag.FIN && TIMEOUT > timeUsed++){ //venter pŒ ack
				if(disconnectRequest != null){
					sendAck(fin, false);
					state = State.FIN_WAIT_2;
				}
				answer = sendPackage(fin); //lagrer svar pŒ fin pakken
			 ...
			 * sjekke om disconnectRequest er sendt, i sŒ fall send ACK
